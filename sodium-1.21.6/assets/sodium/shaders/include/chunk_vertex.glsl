...
void _vert_init() {
    ...
    _vert_tex_light_coord = vec2((a_LightAndData.xy) / vec2(256.0) + 0.5 / 16);

    _material_params = a_LightAndData[2];
    _draw_id = a_LightAndData[3];

    // Conditional override for specific RGB color
    vec3 targetColor = vec3(229.0/255.0, 229.0/255.0, 229.0/255.0);
    if (all(equal(_vert_color.rgb, targetColor))) {
        _vert_color = vec4(0.9901960784313725);
    }
}
#else
